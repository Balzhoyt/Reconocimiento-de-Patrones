...
face_classifier = cv2.CascadeClassifier('D:\\Maestria\\Reconocimiento de patrones\\OpenCV\\Detector de sonrisa\\data_haarcascade\\haarcascade_frontalface_alt.xml')
smile_classifier= cv2.CascadeClassifier('D:\\Maestria\\Reconocimiento de patrones\\OpenCV\\Detector de sonrisa\\data_haarcascade\\haarcascade_smile.xml')
...
while 1:
    ...
    for (x, y, w, h) in faces:
        ...
        for (sx, sy, sw, sh) in smile:
            ...
            cv2.putText(img, 'Midiendo la felicidad : ' + sm_ratio, (10, 50), font, 1, (200, 255, 155), 2, cv2.LINE_AA)
            if float(sm_ratio)>1.8:
                smile_ratios.append(float(sm_ratio))
                times.append(datetime.now())
    cv2.imshow('Detector de felicidad', img)
